<!DOCTYPE html>
<html lang="en" >
<?php
include 'config.php';

$con = mysqli_connect($host, $user, $password,$dbname);

session_start(); 
if(!$_SESSION['login_user']){ 
  header("Location:login.php"); 
  exit;
    }
    if(isset($_SESSION['login_user'])){ 
      if (is_numeric($_SESSION['login_user']))
      { header("Location:login.php"); 
        exit;
      }
  
  }
if (!$con) {
    die("Connection failed: " . mysqli_connect_error());
   }
   else{
   

    $sql3="SELECT w_id,main_pic_path,bride_name, groom_name,rand_no,cr_date,pay_suc,w_plan FROM `invyt_form_org` WHERE c_id= '".$_SESSION['login_user']."'";
    
    $det = mysqli_query($con,$sql3);
   
    
    mysqli_close($con);
}

$nm= explode('@', $_SESSION['login_user'], 2)

?>
<head>
  <meta charset="UTF-8">
  <title>Dashboard</title>
  <meta name="viewport" content="width=device-width, initial-scale=1">
  <link href="https://fonts.googleapis.com/css?family=Dancing+Script" rel="stylesheet">
  <link rel='stylesheet' href='https://cdnjs.cloudflare.com/ajax/libs/twitter-bootstrap/3.3.7/css/bootstrap.min.css'>
 <link href="https://fonts.googleapis.com/css?family=Old+Standard+TT" rel="stylesheet">
 <link href="https://fonts.googleapis.com/css?family=Montserrat" rel="stylesheet">
 <link rel='stylesheet prefetch' href='https://cdnjs.cloudflare.com/ajax/libs/twitter-bootstrap/3.3.7/css/bootstrap.min.css'>
 <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/4.7.0/css/font-awesome.min.css">
 <link rel="stylesheet" href="css/style.css">
 <link rel="stylesheet" href="css/style2.css">
<link rel="stylesheet" href="dashcss/style.css">

  
</head>

<body>

<div class="center-content">
  <header class="top-nav">
    <a class="header__logo"  href="index.php"><img class="header__img" src="img/logo.png"></a> 
    <a href="logout.php"  class="user__img right" data-toggle="tooltip" data-placement="bottom" title="Logout" align="right"></a>
  </header>

  
  
<div class="files-cont">
 <!-- recenti-->    
    <div class="row row--no-right">
        <div class="row__title-box">
           <h2 class="row__title" style="font-family: 'Montserrat', sans-serif; font-size: 25px;">Welcome back, <?php echo $nm[0] ?> !</h2>
        </div>
     </div>
<div class="row row--no-right">
    

 



 <div class="row row--margin">
 <?php
 $no=0;


 while($row = mysqli_fetch_array($det)) {
  $n=$row["w_id"];
  $eno=$n;
  if($row["cr_date"]!=''){
  $cr= explode(' ', $row["cr_date"]);
  }
  else{
      $cr=explode(' ', "_ _");
  }
  $sl="/";
  //$im=str_replace(" ","",$row["main_pic_path"]);
           echo'

   <div class="col-md-6 article-pre__col">
      <a style="text-decoration: none;"';if($row['pay_suc']==1 ){echo 'href="https://invyt.in/'.$row["groom_name"].'weds'. $row["bride_name"].'_'.$row["rand_no"].'.html"';} echo 'target="_blank">
      <div class="article-pre " style="background: url(uploads/'.$row["main_pic_path"].') no-repeat center center; background-size: cover; background-position: 0px 0px; height: 300px; width: 100%;border-radius: 6px; box-shadow: 0 4px 10px 2px rgba(0,0,0,0.3);">
        
        <center><h1 class="article-pre__title" style="font-family:';echo" 'Old Standard TT'";echo ', serif; color:#ffffff; text-shadow: 2px 2px 4px #0f0f0f;">'.$cr[0]. '&emsp; '.$cr[1]. '</h1><br><br><br><br><br><br><br><br><br>
                <h1 class="article-pre__title" style="font-size: 20px; font-family:';echo" 'Montserrat'";echo ', cursive; color:#ffffff;text-shadow: 2px 2px 4px #0f0f0f;">'.$row["groom_name"].' weds '. $row["bride_name"].'</h1>
          
          </center>
      </div></a><br>
        <center>';
        if($row['pay_suc']==1 ){
          $iu="https://invyt.in/".$row["groom_name"]."weds". $row["bride_name"]."_".$row["rand_no"].".html";
          $iu = str_replace(' ', '%20', $iu);
         echo '<a href="finish.php?i_url='.$iu.'"  class="user__img1" data-toggle="tooltip" data-placement="bottom" ></a>';
         
        }
        else{
         echo '<a href="#0" class="cd-popup-trigger user__img1" data-toggle="tooltip" data-placement="bottom"></a> <div class="cd-popup" role="alert">
         <div class="cd-popup-container">
           <p>Please complete payment to share the invyt</p>
           <ul class="cd-buttons">
             <li><a href="update.php?row_id='.$row["w_id"].'">Yes</a></li>
             <li><a href="#0" id="close">No</a></li>
           </ul>
           <a href="#0" class="cd-popup-close img-replace">Close</a>
         </div> 
         </div>
';
        }
        echo '<a href="';if($row["w_plan"]==2){echo 'p2/p2_update.php?row_id='.$row["w_id"].'"';}else{echo 'update.php?row_id='.$row["w_id"].'"';};echo '"  class="user__img2" data-toggle="tooltip" data-placement="bottom"></a>
        <a href="del.php?row_id='.$row["w_id"].'" onclick="return confirm(';echo "'Are you sure?')"; echo '"  class="user__img3" data-toggle="tooltip" data-placement="bottom"></a>
        </center><br><br>
    </div>  
       
';


 $no++;
 if($no%2==0){
   echo '<br>';
 }
}
?>

     </div>
</div>
</div>
    
<div class="floating-container">
  <a href="form.php" class="floating-button"><p style="color: #ffffff; font-size:25px;" title="Create new invyt">+</p></a>
</div>
</div>
<footer class="footer">
  <div class="container">
    <div class="row">
      <div class="col-md-2 col-xs-6">
       <a class="header__logo"  href="index.php"><img class="header__img" src="https://image.ibb.co/fd2dsK/logo2_1.png"></a></div>
      <!--<div class="col-md-10 col-xs-6">
        <div class="footer__social">
          <a href="#" class="footer__social-l">
            <img src="data:image/svg+xml;utf8;base64," />
          </a>
          <a href="#" class="footer__social-l">
            <img src="data:image/svg+xml;utf8;base64," />
          </a>
          
           <a href="#" class="footer__social-l">
          <img src="data:image/svg+xml;utf8;base64," />
          </a>
          
          <a href="" class="footer__social-l">
            <img src="data:image/svg+xml;utf8;base64," />
          </a>
        </div>
      </div>-->
    </div>
  </div>
</footer>

   

<script src='https://cdnjs.cloudflare.com/ajax/libs/jquery/3.3.1/jquery.js'></script>
<script src='https://cdnjs.cloudflare.com/ajax/libs/twitter-bootstrap/3.3.7/js/bootstrap.min.js'></script>
<script src='https://cdnjs.cloudflare.com/ajax/libs/jquery/3.2.1/jquery.min.js'></script>
<script src='https://cdnjs.cloudflare.com/ajax/libs/twitter-bootstrap/3.3.7/js/bootstrap.min.js'></script>
<script  src="js/popup.js"></script>

  

    <script  src="js/index.js"></script>
  

    <script  src="dashjs/index.js"></script>




</body>

</html>
